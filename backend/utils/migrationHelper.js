...
const newInstance = async (model, image, user) => {
  ...
  const engineCost = (model.region === 'worldwide' && machineType !== null && image.provider !== 'Google') ? await billingHelper.getEnginePrice(machineType.metadata.name.substring(0,2).toUpperCase(), machineType.metadata.guestCpus, machineType.metadata.memoryMb / 1024) : {cost: 0, region: 'whatever' }
